...
large_m <- large_2 %>%
    mutate(grand_mean_cpue_wt = mean(cpue_wt), grand_mean_cpue_cnt = mean(cpue_cnt),
           gear_code = factor(gear_code), haul = factor(haul))  #levels=c(1,5) for gear_code if looking to correct old to new. Right now this corrects new to old
    

small_m <- small_2 %>%
    mutate(grand_mean_cpue_wt = mean(cpue_wt), grand_mean_cpue_cnt = mean(cpue_cnt),
           gear_code = factor(gear_code), haul = factor(haul)) #levels=c(1,5) if looking to correct old to new. Right now this corrects new to old
...
shapiro.test((large_m$cpue_wt+1))
shapiro.test((small_m$cpue_wt+1))
...
plot(mod1_small_wt) #24, 35, 36, and 23 are outliers. These are hauls 41(indices 23 and 24) and 71(indices 35 and 36)
plot(mod1_large_cnt)
plot(mod1_small_cnt)
```

---
title: "RMD_scallops_7_presentable"
author: "Alex Reich"
date: "2023-09-27"
...
#TAke away
I get different values for FPC based on this:options(contrasts = rep("contr.sum", 2)). I do not really undserstand what this does or why I should use it. But it is used in the crab code, so I probably should use it. But, does it do what I think it should do? Does it accept my data the way that I think that it is? Do I need to wrangle the data more specifically before using it?
...
- when that options code is applied, it changes the estimate for the model output but not the signficance. Probs changes the coeff interpretations. But how exactly, and which one do I use to calculate FPC? Because they will give me different FPC values.The intercept value is also the same between the two methods
...
- a higher precision. See table. After removing the outliers, both small and large scallops have confidence intervals outside of 1.
...
Graph fitted values 
```{r}
#graph_predicted_cpue_wt<- function(mod, data){
#newdata <- data.frame(gear_code_nd= base::rep(c("1","5")))
#newdat <- data.frame(gear_code=data$gear_code, haul=data$haul)
#data$predicted_values <-predict(object=mod, newdata=newdat)
#print(predicted_values)


#ggplot(data) + aes(x=haul, shape=gear_code, group= gear_code) + 
 #   geom_point(y=cpue_wt, color="black", alpha=0.5)+
  #  geom_point(y=predicted_values, color="red") 
#}

#graph_predicted(mod=mod1_large_wt, data=large_m)
...
########################################
Kappenman exploration. NOTE: after fpc(method=2) is run, lm() does not work the same...
```{r}
#large

#sigh. Does not work below.
#fpc does not work the same as my randomized bloc anova and I am confused as to why. See tyler code. 
...
#ANOVA_fpc_l_wt_cheating2 <- fpc(a, b, method = 2, kapp_zeros = "ind", boot_type = "unpaired")
#might not run correctly unless the gear_type is in order by haul. Right now it is not.

#ANOVA_fpc_l_wt_cheating #ok. the shortcut method seems identical to the lm method 
#FPC_large_wt #the inverse, in this case
...
Create lm function to plug into the mse sim WIP!!!!! - DELETE THIS BLOCK; DONT RUN
```{r}
f_lm_AR_wt <- function(data) {
  fun <- lm(log(cpue_wt+1) ~ gear_code + haul ,data)
...
MSE sim lm ANOVA function 1-WIP!!!!!!!!!!!!!!!!!!!!!!!! DONT RUN
I'm not sure how to set up the MSE for a randomized bloc ANOVA, with fpc messing up.
```{r}
#AR adjustments
f_lm_mse_sim_wt_AR1 <- function(data, n){
  ...
  # compute parameters of lognormal dist
  mu = log(mean(cpue)) - 0.5*log(1+(sd(cpue)/mean(cpue))^2)
  var = log(1+(sd(cpue)/mean(cpue))^2) #for the bootstrap. AR.
  ...
  for(i in 1:length(fpd)){
    for(j in 1:200){
      ...
      # simulate cpue non-standard vessel aka the Kodiak dredge
      kod_sim = rlnorm(n, meanlog = mu, sdlog = sqrt(var)) * fpd[i] #simulations for KODIAK
      # estimate FPC WITH LM
      fpc <- fpc(cpue1 = hom_sim, cpue2 = kod_sim, method = method, nboot = 0, kapp_zeros = "ind") #ah. here we can choose if we want method 2 (anova) or 4(kappenmans)
      ##CAREFUIL!!  RES_SIM AND SOL_SIM ABOVE NEED TO BE REPLACED WITH WHAT IS MEANINGFOR TO YOUR VESSEL
      
      #10/05/23 - AR -  I'm tripping over what to do here. DO I run a regular lm to calculate an FPC? How would I do randomized block? Alternatively, how do I make the dang fpc code work?? ORdering by haul did not seem to work?
...
A non-QC'ed Attempt from stackexchange:
```{r}
library("spatstat")
?ewcdf

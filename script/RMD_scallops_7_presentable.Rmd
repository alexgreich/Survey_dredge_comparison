...
Apply an FPC for large scallops, but not for small scallops. The size distribution of catch between the Homer and Kodiak dredges are similar, according to the Komologorov-Smirnof test. I've identified 2 outliers for the large scallops and 4 outliers for the small scallops. STATE RESULTS FOR ANOVA AND MSE

I also tried the KAppenman's method. It had more conservative results. STATE RESULTS FOR KAPP AND MSE. This test does not assume normality, which is a plus. However, CPUE's of 0 are deleted, which is undesirable here. STATE KAPPENMAN RESULTS.

Normality: does not look terrible terrible when graphed but according to the Shaprio-Wilk test, both large and small scallop datasets are non-normal. When outliers are removed, the large scallop dataset is non-normal and the small scallop dataset is normal, according to the Shapiro-Wilk 
...
expand_grid(logbook,
            distinct(transmute(catch_raw, samp_grp, rcode, comname, whole_haul, sample_type))) %>% #expand logbook to all catch types
...
large_m <- large_2 %>%
           mutate(gear_code = factor(gear_code), haul = factor(haul))  #levels=c(1,5) ? Specify levels in factors?
    

small_m <- small_2 %>%
           mutate(gear_code = factor(gear_code), haul = factor(haul)) #levels=c(1,5) ?
...
ANOVA_fpc_l_wt_cheating <- fpc(cpue2, cpue1, method = 2, kapp_zeros = "ind", boot_type = "unpaired")

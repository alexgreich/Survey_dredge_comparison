...
---
This version is adjusted so the FPC will be to correct Kodiak to Homer values, since most of the time series was collected by the Homer dredge.

Older version with a mix of this: RMD_scallops_7.RMD
...
large_m <- large_2 %>%
           mutate(gear_code = factor(gear_code), haul = factor(haul))  #levels=c(1,5) for gear_code if looking to correct old to new. Right now this corrects new to old
    

small_m <- small_2 %>%
           mutate(gear_code = factor(gear_code), haul = factor(haul)) #levels=c(1,5) if looking to correct old to new. Right now this corrects new to old
...
##large scallops
#large_m$gear_code <- relevel(large_m$gear_code, ref="5")

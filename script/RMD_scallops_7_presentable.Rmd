...
NEXT:
-what are my ANOVA results when I remove the outliers?
...
large_m <- large_2 %>%
           dplyr::mutate(gear_code = base::factor(gear_code), haul = base::factor(haul))  #levels=c(1,5) ? Specify levels in factors?
    

small_m <- small_2 %>%
           dplyr::mutate(gear_code = base::factor(gear_code), haul = base::factor(haul)) #levels=c(1,5) ?
...
I have been having consistency issues with lm. Right now it is using gear_type 5 as base and gear_type 1 as change. Sometimes it does the opposite. Not sure why
...
ANOVA_fpc_l_wt_cheating <- fpc(log(a+1), log(b+1), method = 2, kapp_zeros = "ind", boot_type = "unpaired")

...
MSE sim Kapp - adjsuted from crab github code #DOES THIS WORK?? I just switched from a homer base perspective to a kodiak base perspective.
```{r}
#AR adjustments
f_kapp_mse_sim_AR1 <- function(data, n, method){
  ...
  for(i in 1:length(fpd)){
    for(j in 1:200){
      ...
      kod_sim = rlnorm(n, meanlog = mu, sdlog = sqrt(var)) #simulations for HOMER
      # simulate cpue non-standard vessel aka the Kodiak dredge
      hom_sim = rlnorm(n, meanlog = mu, sdlog = sqrt(var)) * fpd[i] #simulations for KODIAK
      # estimate FPC
      fpc <- fpc(cpue1 = kod_sim, cpue2 = hom_sim, method = method, nboot = 0, kapp_zeros = "ind") #ah. here we can choose if we want method 2 (anova) or 4(kappenmans)
...
#wrong because the order is wrong

fitted_kodiak_l <- large_m %>% filter(gear_code==5) %>%
    dplyr::select(fitted_cpue_wt)
fitted_homer_l <- large_m %>% filter(gear_code==1) %>%
    dplyr::select(fitted_cpue_wt)



fitted_kodiak_s <- small_m %>% filter(gear_code==5) %>%
    dplyr::select(fitted_cpue_wt)
fitted_homer_s <- small_m %>% filter(gear_code==1) %>%
    dplyr::select(fitted_cpue_wt)

#WRONG BELOW - becasue the order is wrong
restructured_data_l$fitted_kodiak <- fitted_kodiak_l
restructured_data_l$fitted_homer <- fitted_homer_l

restructured_data_s$fitted_kodiak <- fitted_kodiak_s
restructured_data_s$fitted_homer <- fitted_homer_s

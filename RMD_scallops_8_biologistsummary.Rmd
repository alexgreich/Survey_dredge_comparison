...
ggplot(restructured_data_l) + 
    #geom_abline(intercept=0, slope=1, linetype="dashed", color= "red")+
    geom_smooth(aes(x=log(Kodiak_cpue_wt+1), y=log(Homer_cpue_wt+1))) + geom_point(aes(x=log(Kodiak_cpue_wt+1), y=log(Homer_cpue_wt+1), color=Haul, shape=Reg_Area),size=3) + 
    theme_cowplot() +
    scale_color_viridis(discrete=T)+
    scale_shape_manual(values=c(19, 17, 15), labels=c("D", "E", "K"))+
    xlab("Kodiak CPUE (weight)")+
...
ggplot(restructured_data_s) + 
    #geom_abline(intercept=0, slope=1, linetype="dashed", color= "red")+
    geom_smooth(aes(x=log(Kodiak_cpue_wt+1), y=log(Homer_cpue_wt+1))) + 
